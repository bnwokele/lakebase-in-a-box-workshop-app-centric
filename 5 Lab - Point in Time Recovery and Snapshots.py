...
# MAGIC
# MAGIC Now that you've evolved the OLTP schema in production (Lab 4), the storefront is running its
...
# MAGIC > to a point in time before a later schema change added the `priority` column. This is expected behavior
...
# MAGIC the schema migration in Lab 4 — idempotent DDL that's safe to run multiple times.
# MAGIC
...
POST_RECOVERY_SQL = f"""
-- Post-recovery schema change: Add email_verified to customers
...
-- Post-recovery schema change: Add priority to orders
...
# MAGIC - All features from the earlier schema migrations are restored
# MAGIC
...
# MAGIC ---
# MAGIC
# MAGIC **Previous:** Lab 4 — Schema Migration | **Next:** Lab 6 — Reverse ETL with Synced Tables
# MAGIC
