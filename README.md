...
real time as each lab changes the database.
...
| 3 | `3 Lab - Parallel Development with Branching` | Zero-copy branching; three developers evolve the schema in parallel on isolated branches (loyalty + reviews, multi-currency FK, performance indexes) |
| 4 | `4 Lab - Schema Migration to Production` | Promote validated changes from a feature branch to production via Migration Replay; Schema Diff and Branch Reset concepts |
| 5 | `5 Lab - Point in Time Recovery and Snapshots` | Simulate an accidental `DROP TABLE` and recover production with PITR; snapshots vs. PITR |
| 6 | `6 Lab - Reverse ETL with Synced Tables (UC to Lakebase)` | Push Spring Sale promotions from a Unity Catalog Delta table into Lakebase via managed synced tables |
| 7 | `7 Lab - Lakebase CDF (Lakebase to UC)` | Continuously mirror live Lakebase tables into Delta in Unity Catalog for high-throughput analytics |
...
| Analytics surface (no storefront change) | Lab 7 (Lakebase CDF) |
...
- **Sale deals not appearing (after Lab 6)** — synced tables are created by the sync pipeline, so
  ...
  TO "<SP_CLIENT_ID>";` step from Lab 6.
- **Logs:** `<app-url>/logz`
