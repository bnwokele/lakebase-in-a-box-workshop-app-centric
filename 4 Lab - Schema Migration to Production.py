...
# MAGIC
# MAGIC In Lab 3, we created a feature branch `dev-loyalty-reviews`, added the `loyalty_points` column to the `customers` table, created new `loyalty_members` and `reviews` tables. Now we'll promote those changes to production using the **Migration Replay** pattern.
# MAGIC
...
# MAGIC
# MAGIC **Next:** In Lab 5, we'll explore **Point-in-Time Recovery & Snapshots**.
