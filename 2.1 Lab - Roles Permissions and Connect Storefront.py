...
# MAGIC ---
# MAGIC
# MAGIC This lab provides a comprehensive introduction to how **permissions** work in Lakebase, covering the two main layers: **workspace** and **database**. You'll then apply this knowledge hands-on by granting the DataCart Storefront app access to your Lakebase project.
# MAGIC
...
# MAGIC 6. **Connect a Databricks App** (the DataCart Storefront) to Lakebase via its service principal
# MAGIC
# MAGIC ## Where We Are
# MAGIC
# MAGIC 1. **Lab 1** created a Lakebase Autoscaling project and seeded the `ecommerce` schema
# MAGIC    with 5 tables: `customers`, `products`, `inventory`, `orders`, and `order_items`.
# MAGIC
# MAGIC 2. **The DataCart Storefront app** has been deployed as a Databricks App
# MAGIC    (via the UI, DABs, or CLI — see the Workshop Setup guide). The app is running
