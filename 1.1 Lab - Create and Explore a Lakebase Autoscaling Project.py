...
# MAGIC %md
# MAGIC # Lab 1: Create and Explore a Lakebase Autoscaling Project
# MAGIC
# MAGIC This notebook provides a comprehensive, hands-on introduction to **Databricks Lakebase Autoscaling PostgreSQL**. You will create a Lakebase project, explore its configuration, connect to it, seed it with a realistic e-commerce dataset, and explore PostgreSQL system metadata.
# MAGIC
# MAGIC ## Learning Objectives
# MAGIC
# MAGIC By the end of this lab, you will be able to:
# MAGIC 1. **Explain** what a Lakebase Autoscaling project is and how it differs from Lakebase Provisioned
# MAGIC 2. **Provision and configure** a Lakebase Autoscaling project using the Databricks SDK
...
# MAGIC 5. **Explore PostgreSQL system metadata** through `pg_catalog` and `information_schema`
# MAGIC 6. **Understand** how to query Lakebase from the Lakehouse (direct connect vs. Unity Catalog registration)
# MAGIC
...
# MAGIC 5. Explores PostgreSQL system metadata (`pg_catalog`, `information_schema`)
# MAGIC 6. Verifies everything is ready for the remaining workshop labs
# MAGIC
...
# MAGIC
# MAGIC We'll create a new Lakebase project with autoscaling compute using the Databricks SDK.
# MAGIC
# MAGIC **What happens under the hood when a project is created:**
# MAGIC - A new **PostgreSQL 17** instance is provisioned
# MAGIC - A default **`production` branch** is created automatically
# MAGIC - A **primary read-write compute** endpoint is attached to the `production` branch
# MAGIC - **Autoscaling** is configured between your min and max CU values
# MAGIC - The compute **auto-suspends** after the configured idle timeout
# MAGIC - A **Postgres role** for your Databricks identity (e.g., `user@databricks.com`) is automatically created
# MAGIC - A default **`databricks_postgres`** database is created
# MAGIC
# MAGIC > You can also create a project through the UI by navigating to **Lakebase Postgres** in the apps switcher and clicking **New project**.
# MAGIC
# MAGIC > **Docs:** [Create a project](https://docs.databricks.com/aws/en/oltp/projects/)
...
# MAGIC - **Provisioned** a Lakebase PostgreSQL 17 project with autoscaling compute via the Databricks SDK
...
# MAGIC - **Explored system metadata** through `pg_catalog` and `information_schema`, confirming Lakebase is standard PostgreSQL
# MAGIC - **Understood** how to query Lakebase from the Lakehouse via direct connection and Unity Catalog registration
# MAGIC
# MAGIC Your project is now ready for the remaining workshop labs on **branching**, **parallel development**, **point-in-time recovery**, and **reverse ETL**.
